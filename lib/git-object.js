...
class GitCommit extends GitObject {
  constructor(gitDirPath, type, size, hash, body) {
    ...
    }
    {
      ...
      if (m[0]) {
        this.parentHash = m[0].split(/\s/).pop();
      }
      if (m[1]) {
        this.mergedParentHash = m[1].split(/\s/).pop();
      }
    }
  }

  isMergeCommit() {
    return !!this.mergedParentHash;
  }
  ...
  async readTree() {
    if (!this.hasTree()) throw new Error('This commit has no trees.');
    return await readGitObject(this._gitDirPath, this.treeHash);
  }

  async readParent() {
    if (!this.hasParent()) throw new Error('This commit has no parents.');
    return await readGitObject(this._gitDirPath, this.parentHash);
  }

  async readMergedParent() {
    if (!this.isMergeCommit()) throw new Error('This commit is not a merge commit.');
    return await readGitObject(this._gitDirPath, this.mergedParentHash);
  }
}
